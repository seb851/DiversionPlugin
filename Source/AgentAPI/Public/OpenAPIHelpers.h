...
class AGENTAPI_API HttpFileInput
{
public:
...
AGENTAPI_API bool ParseDateTime(const FString& DateTimeString, FDateTime& OutDateTime);
...
inline bool TryGetJsonValue(const TSharedPtr<FJsonValue>& JsonValue, TSharedPtr<FJsonObject>& JsonObjectValue)
{
	...
	{
		JsonObjectValue = *InnerGeneratorOpenAPIObject;
		return true;
	}
	return false;
}
...
inline bool TryGetJsonValue(const TSharedPtr<FJsonValue>& JsonValue, TMap<FString, T>& MapValue)
{
	...
	if (JsonValue->TryGetObject(InnerGeneratorOpenAPIObject))
	{
		MapValue.Reset();
		bool ParseSuccess = true;
		for (const auto& It : (*InnerGeneratorOpenAPIObject)->Values)
		{
			T TmpValue;
