...
void Inline_object_1::WriteJson(JsonWriter& Writer) const
{
	Writer->WriteObjectStart();
	Writer->WriteIdentifierPrefix(TEXT("object")); WriteJsonValue(Writer, Inline_object_1::EnumToString(mobject));
	Writer->WriteIdentifierPrefix(TEXT("items")); WriteJsonValue(Writer, mItems);
	Writer->WriteObjectEnd();
}

bool Inline_object_1::FromJson(const TSharedPtr<FJsonValue>& JsonValue)
{
    ...
    if (ParseEnumobjectStringSuccess) {
        ParseSuccess &= Inline_object_1::EnumFromString(objectString, mobject);
...
FString Inline_object_1::EnumToString(const ObjectEnum& EnumValue) {
    switch (EnumValue)
    {
    case ObjectEnum::BRANCH:
        return TEXT("BRANCH");
    default:
        return TEXT("");
    }
...
bool Inline_object_1::EnumFromString(const FString& EnumAsString, ObjectEnum& EnumValue) {
    if(EnumAsString.IsEmpty()) return false;
    if(EnumAsString == TEXT("BRANCH")) {
        EnumValue = ObjectEnum::BRANCH;
        return true;
    }
