...
void Inline_object_3::WriteJson(JsonWriter& Writer) const
{
	Writer->WriteObjectStart();
	Writer->WriteIdentifierPrefix(TEXT("object")); WriteJsonValue(Writer, Inline_object_3::EnumToString(mobject));
	Writer->WriteIdentifierPrefix(TEXT("items")); WriteJsonValue(Writer, mItems);
	Writer->WriteObjectEnd();
}

bool Inline_object_3::FromJson(const TSharedPtr<FJsonValue>& JsonValue)
{
    ...
    if (ParseEnumobjectStringSuccess) {
        ParseSuccess &= Inline_object_3::EnumFromString(objectString, mobject);
...
FString Inline_object_3::EnumToString(const ObjectEnum& EnumValue) {
    switch (EnumValue)
    {
    case ObjectEnum::COMMIT:
        return TEXT("COMMIT");
    default:
        return TEXT("");
    }
...
bool Inline_object_3::EnumFromString(const FString& EnumAsString, ObjectEnum& EnumValue) {
    if(EnumAsString.IsEmpty()) return false;
    if(EnumAsString == TEXT("COMMIT")) {
        EnumValue = ObjectEnum::COMMIT;
        return true;
    }
