...
void Inline_object_2::WriteJson(JsonWriter& Writer) const
{
	Writer->WriteObjectStart();
	Writer->WriteIdentifierPrefix(TEXT("object")); WriteJsonValue(Writer, Inline_object_2::EnumToString(mobject));
	Writer->WriteIdentifierPrefix(TEXT("items")); WriteJsonValue(Writer, mItems);
	Writer->WriteObjectEnd();
}

bool Inline_object_2::FromJson(const TSharedPtr<FJsonValue>& JsonValue)
{
    ...
    if (ParseEnumobjectStringSuccess) {
        ParseSuccess &= Inline_object_2::EnumFromString(objectString, mobject);
...
FString Inline_object_2::EnumToString(const ObjectEnum& EnumValue) {
    switch (EnumValue)
    {
    case ObjectEnum::WORKSPACE:
...
bool Inline_object_2::EnumFromString(const FString& EnumAsString, ObjectEnum& EnumValue) {
    if(EnumAsString.IsEmpty()) return false;
    if(EnumAsString == TEXT("WORKSPACE")) {
        EnumValue = ObjectEnum::WORKSPACE;
        return true;
    }
